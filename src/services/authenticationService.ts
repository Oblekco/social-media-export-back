...
export const authenticateUser = async (
    ...
    try {
        ...
        const [rows] = await connection.query(
            "SELECT id FROM AUDITORIA_USUARIOS WHERE username = ? AND password = ? AND rol = 'ADMIN' LIMIT 1",
            [username, password]
        );
...
export const getUserEmailById = async (id: string): Promise<string | null> => {
    try {
        const connection = await dbProdConnection();
        const [rows] = await connection.query(
            "SELECT email FROM AUDITORIA_USUARIOS WHERE id = ? LIMIT 1",
            [id]
        );
...
export const findUserById = async (id: DecodedToken): Promise<User | null> => {
    try {
        const connection = await dbProdConnection();
        const [rows] = await connection.query(
            "SELECT id, username, email, rol FROM AUDITORIA_USUARIOS WHERE id = ? LIMIT 1",
            [id.id]
        );
